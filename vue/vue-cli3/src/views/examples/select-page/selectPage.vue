    ...
    <div class="body">
      ...
      <div class="hobby" v-show="page.index === '2'">
        ...
        <p>
          ...
          <p>
            <input type="checkbox" value="swimming" v-model="page.hobby.value">
            <label>游泳</label>
          </p>
          <p>
          ...
          <p>
            <Button @click.native="switchPage(1)" :active=true style="margin-right: 5px">上一步</Button>
            <Button @click.native="switchPage(3)" :active=false style="margin-right: 5px">下一步</Button>
            <Button @click.native="reset()" :active=false>重置</Button>
          </p>
      </div>
      <div class="introduce" v-show="page.index === '3'">
        <p>1.介绍一下自己</p>
        <textarea name="" id="" cols="30" rows="10" v-model="page.secrecy.value"></textarea>
        <p>
          {{isSubmit}}
          <Button @click.native="submit()" :active=true style="margin-right: 5px" :disabled="isSubmit">提交</Button>
          <Button @click.native="switchPage(2)" :active=false style="margin-right: 5px">上一步</Button>
          <Button @click.native="reset()" :active=false>重置</Button>
        </p>
      </div>
    </div>
